...
public sealed class Bm8PenaltyPrototype : MonoBehaviour
{
    ...
    private IEnumerator FlyBall(Vector3 from, Vector3 to, bool saved, float duration)
    {
        ...
        while (elapsed < duration)
        {
            ...
            if (saved && catchAaSave)
            {
                ball.position = ClampAaCatchBallToHand(ball.position, t, contactTime);
            }
    ...
    private void ApplyLowSideSaveSettleMotion(float actionT)
    {
        ...
        float dive = Smooth(Mathf.Clamp01(Mathf.InverseLerp(0.44f, 0.72f, actionT)));
        if (dive <= 0.001f)
        {
            return;
        }
        ...
        float bodyDrop = Smooth(Mathf.Clamp01(Mathf.InverseLerp(0.5f, 0.82f, actionT)));
        keeperVisibleModel.position += new Vector3(side * 0.1f * dive, -0.12f * bodyDrop, -0.08f * dive);
        keeperVisibleModel.rotation = Quaternion.Slerp(
            keeperVisibleModel.rotation,
            keeperVisibleModel.rotation * Quaternion.Euler(10f * dive, 0f, -side * 16f * dive),
            0.68f);
    }
    ...
    private void ApplyImportedKeeperActionOffset(float t)
    {
        ...
        else if (bottom && side != 0f && keeperCurrentSave)
        {
            recover = 0f;
            hold = launch * (1f - recover);
            contactPunch = Mathf.Sin(Mathf.Clamp01(Mathf.InverseLerp(0.28f, 0.56f, t)) * Mathf.PI);
        }
        ...
        {
            x = -visualSide * 0.14f * anticipation - visualSide * 0.05f * readDrop - visualSide * 0.1f * coil + visualSide * 1.28f * hold + visualSide * 0.08f * contactPunch;
            y = -0.22f * anticipation - 0.1f * readDrop - 0.08f * coil - 0.54f * hold + 0.035f * snap;
            z = 0.1f * anticipation + 0.02f * readDrop - 0.04f * coil - 0.68f * hold - 0.04f * contactPunch;
        }
        if (side == 0f)
        {
            x = 0f;
        ...
        {
            pitch = 10f * anticipation + 4f * readDrop - 4f * coil + 28f * hold + 8f * contactPunch;
            roll = visualSide * 6f * anticipation + visualSide * 3f * readDrop - visualSide * (58f * hold + 10f * contactPunch);
        }
    ...
    private struct AaKeeperMotionProfile
    {
        ...
        public static AaKeeperMotionProfile For(int col, int row)
        {
            ...
            return new AaKeeperMotionProfile
            {
                ...
                rootSide = center ? 0f : 1.16f,
                rootZ = center ? -0.46f : -0.42f,
                rootPitch = center ? 4f : 8f,
                rootYawSide = 9f,
                rootRollSide = center ? -12f : -16f,
    ...
    private Vector3 AaDeflectWorld(Vector3 contact, float reboundSide)
    {
        ...
        {
            return new Vector3(
                Mathf.Clamp(contact.x + reboundSide * 1.05f, -3.15f, 3.15f),
                Mathf.Clamp(contact.y + 0.22f, 1.05f, 3.2f),
                contact.z - 1.02f);
        }
    ...
    private Vector3 AaDeflectDropWorld(Vector3 deflect, float reboundSide)
    {
        bool center = MotionKeeperCol == 1;
        if (!center && MotionKeeperRow == 0)
        {
            return new Vector3(
                Mathf.Clamp(deflect.x + reboundSide * 0.1f, -3.25f, 3.25f),
                0.3f,
                deflect.z - 0.22f);
        }
    ...
    private Vector3 AaCatchDropWorld(Vector3 caught, float reboundSide)
    {
        ...
        if (MotionKeeperCol != 1)
        {
            return new Vector3(
                caught.x,
                MotionKeeperRow == 2 ? 0.24f : 0.62f,
                caught.z - (MotionKeeperRow == 2 ? 0.08f : 0.12f));
        }
    ...
    private Vector3 ClampAaCatchBallToHand(Vector3 plannedPosition, float t, float contactTime)
    {
        ...
        Vector3 handBall = handWorld + AaBallCenterFromHandOffset();
        float follow = Mathf.Clamp01(Mathf.InverseLerp(contactTime, Mathf.Min(0.98f, contactTime + 0.22f), t));
        float maxGap = MotionKeeperCol == 1 ? 0.24f : MotionKeeperRow == 2 ? 0.13f : 0.16f;
        Vector3 delta = plannedPosition - handBall;
        if (delta.magnitude > maxGap)
        {
            plannedPosition = handBall + delta.normalized * maxGap;
        }

        float followWeight = MotionKeeperCol == 1 ? Mathf.Lerp(0.35f, 0.62f, follow) : Mathf.Lerp(0.72f, 0.94f, follow);
        return Vector3.Lerp(plannedPosition, handBall, followWeight);
    }
